...
public class StudentServiceImpl implements StudentService{
    ...
    @Override
    public Iterable<StudentOutputDto> getAllStudentsByNameLike(String name) {
        return studentRepository.findByNameLike("%"+name+"%").stream()
                .map(Student::studentToStudentOutputDto)
                .toList();
    }

    @Override
    public Iterable<StudentOutputDto> getAllStudentsByNameLikeJQPL(String name) {
        return studentRepository.getByNameLike(name).stream()
                .map(Student::studentToStudentOutputDto)
                .toList();
    }

    @Override
    public Iterable<StudentOutputDto> getAllStudentsByNameLikeNative(String name) {
        return studentRepository.getByNameLikeNative(name).stream()
                .map(Student::studentToStudentOutputDto)
                .toList();
    }
